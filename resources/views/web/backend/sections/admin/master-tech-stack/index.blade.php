...
@push('title')
    TransForKr4b | Master tech-stack
@endpush

@section('content')
<div class="p-4 pb-12 pt-20 sm:ml-72">
    <div class="my-4 text-center">
        <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Master Tech Stack <span class="bg-blue-100 text-blue-800 text-2xl font-semibold mr-2 px-2.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-2">Beta</span></h2>
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Tambah Data Tech Stack</h3>
                        <form id="createForm" class="space-y-6" action="#" enctype="multipart/form-data"
                            onsubmit="event.preventDefault(); createData();"
                        >
                            @csrf
                            <div>
                                <label for="name" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Nama Tech Stack</label>
                                <div class="flex">
                            ...
                            <div>
                                <label for="link" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Slug (URL Costum)</label>
                                <div class="flex">
                                    <span class="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                                    ...
                                    <input type="text" id="link" name="link" class="rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="ex: https://laravel.com">
                                    </div>
                            </div>

                            <div class="flex flex-col items-center justify-center w-full">
                                <label for="logo" class="block text-sm font-medium text-gray-900 dark:text-white text-left self-start">Foto / Logo Tech Stack</label>
        ...
        <div id="editModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Data Tech Stack</h3>
...
@push('post-js')
<script>
    const assetUrl = @json(asset('storage/logo/'));
</script>
...
<script>
    ...
    function getData(params, pageUrl = "{{ route('dashboard.master-tech-stack.getData') }}") {
        // Make a GET request using Axios with the provided params
        // axios.get("{{ route('dashboard.master-tech-stack.getData') }}", {
        //     params: params
        // })
        axios.get(pageUrl, {
            params: params
            ...
            .then(function (response) {
                ...
                `
                    <tr class="bg-white text-center border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">${item.link}</td>
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <img class="w-auto h-10 rounded-sm mx-auto" src="${assetUrl+'\/'+item.logo}" alt="Rounded avatar">
                        </td>
                        <td class="px-6 py-4 text-center">
                            <button
                                class="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-teal-300 to-lime-300 group-hover:from-teal-300 group-hover:to-lime-300 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-lime-800"
    ...
    function handleDeleteAction(id) {
        ...
        confirmButton.addEventListener('click', () => {
            ...
            axios
                .post(`{{ route('dashboard.master-tech-stack.delete') }}`, {
                    _method: 'DELETE',
                    id: id
                })
                .then(response => {
                    // Handle the response (e.g., show a success message)
                    console.log('Tech Stack deleted successfully', response);
    ...
    function createData() {
        ...
        axios.post("{{ route('dashboard.master-tech-stack.create') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
                console.log('Tech Stack created successfully', response);
                getData(); // Refresh the table after deletion
    ...
    function updateData() {
        ...
        // Assuming you have a route named 'dashboard.master-tech-stack.update' for updating categories
        axios.post("{{ route('dashboard.master-tech-stack.update') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
