...
@push('title')
    TransForKr4b | Master jurusan
@endpush

@section('content')
<div class="p-4 pb-12 pt-20 sm:ml-72">
    <div class="my-4 text-center">
        <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Master Jurusan <span class="bg-blue-100 text-blue-800 text-2xl font-semibold mr-2 px-2.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-2">Beta</span></h2>
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Tambah Data Jurusan</h3>
                        <form id="createForm" class="space-y-6" action="#" enctype="multipart/form-data"
                            onsubmit="event.preventDefault(); createData();"
                        >
                            @csrf
                            <div>
                                <label for="name" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Nama Jurusan</label>
                                <div class="flex">
                                    ...
                                    <input type="text" id="name" name="name" class="rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="ex: Rekayasa Perangkat Lunak ( RPL )" required>
                                    </div>
                            </div>
                            {{-- <div>
                            ...
                            <div class="flex flex-col items-center justify-center w-full">
                                <label for="logo" class="block text-sm font-medium text-gray-900 dark:text-white text-left self-start">Foto / Logo Jurusan</label>
        ...
        <div id="editModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Data Jurusan</h3>
...
<script>
    ...
    function getData(params, pageUrl = "{{ route('dashboard.master-jurusan.getData') }}") {
        // Make a GET request using Axios with the provided params
        // axios.get("{{ route('dashboard.master-jurusan.getData') }}", {
        //     params: params
        // })
        axios.get(pageUrl, {
            params: params
    ...
    function handleDeleteAction(id) {
        ...
        confirmButton.addEventListener('click', () => {
            ...
            axios
                .post(`{{ route('dashboard.master-jurusan.delete') }}`, {
                    _method: 'DELETE',
                    id: id
                })
                .then(response => {
                    // Handle the response (e.g., show a success message)
                    console.log('Jurusan deleted successfully', response);
    ...
    function createData() {
        ...
        axios.post("{{ route('dashboard.master-jurusan.create') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
    ...
    function updateData() {
        ...
        // Assuming you have a route named 'dashboard.master-jurusan.update' for updating categories
        axios.post("{{ route('dashboard.master-jurusan.update') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
